...
vec3 shadeAmbientLight(Material material, AmbientLight light) {
    ...
    vec3 tex_color = light.color * light.intensity * material.kA;
    vec3 multiple = texture(material.map_kD , TexCoords).rgb;
    tex_color *= multiple;
    return tex_color;
...
vec3 shadeDirectionalLight(Material material, DirectionalLight light, vec3 normal, vec3 eye, vec3 vertex_position) {
    ...
    // Reuse the code from Phong Shading.
    vec3 result = vec3(0);
    if (light.intensity == 0.0)
        return result;
    ...
    result += LN * light.color * light.intensity * material.kD * texture(material.map_kD , TexCoords).rgb;

    // Specular
    vec3 R = reflect(L, N);
    result += pow( max(dot(R, V), 0.0), material.shininess * texture(material.map_nS, TexCoords).r) * light.color * light.intensity * material.kS;
...
void main() {
    ...
    normal = texture(u_material.map_norm, TexCoords).xyz;
